...
if __name__ == "__main__":
    ...
    # --- 新增：并行分片逻辑 ---
    # 从 cfg 中读取 rank 和 world_size，默认为单进程模式
    rank = int(cfg.get("rank", 0))
    world_size = int(cfg.get("world_size", 1))

    # 1. 获取所有场景的 key 列表
    all_scene_names = list(scene_frame_names.keys())
    
    # 2. 必须排序！确保所有进程看到的列表顺序是一致的，否则会乱
    all_scene_names.sort()

    # 3. 根据 rank 和 world_size 进行切片 (类似于 PyTorch DistributedSampler 的逻辑)
    my_scene_names = all_scene_names[rank::world_size]

    logger.info(f"Starting process rank {rank}/{world_size} with GPU isolation.")
    logger.info(f"Total scenes: {len(all_scene_names)} | This rank will process: {len(my_scene_names)}")
    # -----------------------

    # 修改循环，只遍历属于当前 rank 的场景
    for scene_name in tqdm(my_scene_names, desc=f"Rank {rank}"):  
        try:
            scene_root = Path(cfg.root, scene_name)
