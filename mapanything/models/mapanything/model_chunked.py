...
class MapAnythingChunked(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        # Initialize inter-chunk fusion module
        self._initialize_inter_chunk_fusion(inter_chunk_config)
    ...
    def _initialize_inter_chunk_fusion(self, inter_chunk_config):
        """
        Initialize the inter-chunk fusion module based on the configuration.
        遵循与info_sharing相同的初始化模式
        """
        # Initialize Custom Positional Encoding if required
        self.inter_chunk_custom_positional_encoding = inter_chunk_config.get("custom_positional_encoding", None)
        if self.inter_chunk_custom_positional_encoding is not None:
            if isinstance(self.inter_chunk_custom_positional_encoding, str):
                print(
                    f"Using custom positional encoding for inter-chunk fusion: {self.inter_chunk_custom_positional_encoding}"
                )
                if self.inter_chunk_custom_positional_encoding.startswith("RoPE"):
                    rope_freq = float(self.inter_chunk_custom_positional_encoding[len("RoPE"):])
                    print(f"RoPE frequency: {rope_freq}")
                    from uniception.models.libs.croco.pos_embed import RoPE2D
                    self.inter_chunk_custom_positional_encoding = RoPE2D(freq=rope_freq)
            elif isinstance(self.inter_chunk_custom_positional_encoding, Callable):
                print(
                    "Using callable function as custom positional encoding for inter-chunk fusion."
                )
        ...
        # Initialize Inter-Chunk Fusion Transformer
        if inter_chunk_config["model_return_type"] == "no_intermediate_features":
            # Returns only normalized last layer features
            if inter_chunk_config["model_type"] == "cross_attention":
                self.inter_chunk_fusion = MultiViewCrossAttentionTransformer(
                    **inter_chunk_config["module_args"]
                )
            elif inter_chunk_config["model_type"] == "global_attention":
                self.inter_chunk_fusion = MultiViewGlobalAttentionTransformer(
                    **inter_chunk_config["module_args"]
                )
            elif inter_chunk_config["model_type"] == "alternating_attention":
                self.inter_chunk_fusion = MultiViewAlternatingAttentionTransformer(
                    **inter_chunk_config["module_args"]
                )
            else:
                raise ValueError(
                    f"Invalid inter_chunk_model_type: {inter_chunk_config['model_type']}. Valid options: ['cross_attention', 'global_attention', 'alternating_attention']"
                )
        else:
            raise ValueError(
        ...
        if "pose" in self.pred_head_type:
            inter_chunk_pose_config = self.pred_head_config["pose_head"].copy()
            # 修改输入维度为fusion后的特征维度
            inter_chunk_pose_config["input_feature_dim"] = self.info_sharing.dim
            # 初始化PoseHead
            ...
            if identity_init:
                ...
                with torch.no_grad():
                    # 初始化平移输出层为0
                    if hasattr(self.inter_chunk_pose_head, 'fc_t'):
                        torch.nn.init.zeros_(self.inter_chunk_pose_head.fc_t.weight)
                        torch.nn.init.zeros_(self.inter_chunk_pose_head.fc_t.bias)  # [0, 0, 0]
                    
                    # 初始化旋转输出层为单位四元数 [0, 0, 0, 1]
                    if hasattr(self.inter_chunk_pose_head, 'fc_rot'):
                        torch.nn.init.zeros_(self.inter_chunk_pose_head.fc_rot.weight)
                        self.inter_chunk_pose_head.fc_rot.bias.data.copy_(
                            torch.tensor([0.0, 0.0, 0.0, 1.0], device=self.device)
                        )

                print("Initialized inter_chunk_pose_head with identity transformation (progressive init)")
            else:
                # 使用原来的权重复制策略
                if hasattr(self, 'pose_head'):
                    ...
                    with torch.no_grad():
                        try:
                            ...
                            # 加载到inter_chunk_pose_head
                            self.inter_chunk_pose_head.load_state_dict(pose_head_state, strict=False)
                            print("Successfully initialized inter_chunk_pose_head with pose_head weights")
                        except Exception as e:
                            print(f"Warning: Could not copy pose_head weights to inter_chunk_pose_head: {e}")
                            print("Using random initialization for inter_chunk_pose_head")
        else:
            raise ValueError(
                "Inter-chunk fusion pose head can only be initialized if 'pose' is in pred_head_type"
            )
    @property
    ...
    def _transform_chunk_output_to_world(self, res_list, chunk_ref_poses, img_shape):
        ...
        Returns:
            transformed_res_list: List of lists, transformed predictions
        """
        ...
        for chunk_idx, (chunk_res, chunk_ref_pose) in enumerate(zip(res_list, chunk_ref_poses)):
            ...
            if DEBUG:
                ...
                print(f"NOTE: Using identity transform for chunk {chunk_idx} in debug mode.")
                chunk_quats = torch.tensor([0.0, 0.0, 0.0, 1.0], device=chunk_quats.device).expand_as(chunk_quats)
                chunk_trans = torch.zeros_like(chunk_trans)

            for view_res in chunk_res:
                transformed_view_res = {}

                # Copy non-geometric outputs
                for key, value in view_res.items():
                    if key not in ['pts3d', 'ray_origins', 'ray_directions',
                                    'cam_trans', 'cam_quats']:
                        transformed_view_res[key] = value
                ...
                if 'pts3d' in view_res:
                    ...
                    pts3d_chunk = view_res['pts3d']  # (B, H, W, 3)
                ...
                if 'ray_origins' in view_res:
                    ...
                    ray_origins_chunk = view_res['ray_origins']  # (B, H, W, 3)
                ...
                if 'cam_trans' in view_res:
                    ...
                    cam_trans_chunk = view_res['cam_trans']  # (B, 3)
            ...
            transformed_res_list.append(transformed_chunk_res)

        return transformed_res_list
    ...
    def _quaternion_to_rotation_matrix(self, quats):
        ...
        Args:
            quats: (..., 4) tensor of quaternions [x, y, z, w]
    ...
    def _transform_points_to_world(self, points, trans, quats, img_shape):
        """
        Transform points from chunk coordinates to world coordinates.

        Args:
            points: (B, H, W, 3) tensor of points in chunk coordinates
            trans: (B, 3) tensor of translation vectors
            quats: (B, 4) tensor of quaternions
            img_shape: (H, W) image shape

        Returns:
            points_world: (B, H, W, 3) tensor of points in world coordinates
        """
        batch_size, height, width, _ = points.shape

        # Expand trans and quats to match points shape
        trans_expanded = trans.unsqueeze(1).unsqueeze(1).expand(-1, height, width, -1)  # (B, H, W, 3)
        quats_expanded = quats.unsqueeze(1).unsqueeze(1).expand(-1, height, width, -1)  # (B, H, W, 4)

        # Convert quaternions to rotation matrices
        rot_matrices = self._quaternion_to_rotation_matrix(quats_expanded)  # (B, H, W, 3, 3)

        # Apply transformation: points_world = R @ points + t
        points_world = torch.einsum('bhwij,bhwj->bhwi', rot_matrices, points) + trans_expanded

        return points_world

    def _transform_directions_to_world(self, directions, trans, quats, img_shape):
        """
        Transform directions from chunk coordinates to world coordinates (rotation only).

        Args:
            directions: (B, H, W, 3) tensor of directions in chunk coordinates
            trans: (B, 3) tensor of translation vectors (not used for directions)
            quats: (B, 4) tensor of quaternions
            img_shape: (H, W) image shape

        Returns:
            directions_world: (B, H, W, 3) tensor of directions in world coordinates
        """
        batch_size, height, width, _ = directions.shape

        # Expand quats to match directions shape
        quats_expanded = quats.unsqueeze(1).unsqueeze(1).expand(-1, height, width, -1)  # (B, H, W, 4)

        # Convert quaternions to rotation matrices
        rot_matrices = self._quaternion_to_rotation_matrix(quats_expanded)  # (B, H, W, 3, 3)

        # Apply rotation only: directions_world = R @ directions
        directions_world = torch.einsum('bhwij,bhwj->bhwi', rot_matrices, directions)

        return directions_world

    def _transform_pose_to_world(self, cam_trans, cam_quats, chunk_trans, chunk_quats):
        """
        Transform camera pose from chunk coordinates to world coordinates.

        Args:
            cam_trans: (B, 3) tensor of camera translation in chunk coordinates
            cam_quats: (B, 4) tensor of camera quaternions in chunk coordinates
            chunk_trans: (B, 3) tensor of chunk translation
            chunk_quats: (B, 4) tensor of chunk quaternions

            cam_pose = chunk_pose @ cam_pose_in_chunk 

        Returns:
            cam_trans_world: (B, 3) tensor of camera translation in world coordinates
            cam_quats_world: (B, 4) tensor of camera quaternions in world coordinates
        """
        cam_quats = cam_quats / torch.norm(cam_quats, dim=-1, keepdim=True)
        chunk_quats = chunk_quats / torch.norm(chunk_quats, dim=-1, keepdim=True)

        cam_quats_world = quaternion_multiply(chunk_quats, cam_quats)  
        # Normalize the resulting quaternion
        cam_quats_world = cam_quats_world / torch.norm(cam_quats_world, dim=-1, keepdim=True)

        # calculate cam_trans_world
        rot_matrices_chunk = self._quaternion_to_rotation_matrix(chunk_quats)  # (B, 3, 3)
        cam_trans_world = torch.einsum('bij,bj->bi', rot_matrices_chunk, cam_trans) + chunk_trans

        return cam_trans_world, cam_quats_world
    ...
    def forward(self, views, num_chunks: int = 1, memory_efficient_inference: bool = False):
        ...
        1. 将N视角按num_chunks分割成chunks，每个chunk独立encode + info_sharing
        2. 收集所有views的features，作为inter-chunk fusion的输入（形状和原始n views一样）
        3. Inter-chunk fusion：对所有views的features做全局信息共享（不加scale token）
        4. 用fused features预测每个chunk的ref view pose（chunk2world）
        5. 用chunk poses生成统一坐标系的predictions

        Assumption:
        - All the input views and dense geometric inputs have the same image shape.
        - num_views % num_chunks == 0
        - 每个chunk的第一个view是ref view
        ...
        Args:
            ...
            num_chunks (int): Number of chunks to split the views into. Default is 4.
        ...
        if self.inter_chunk_fusion_return_type == "no_intermediate_features":
            fused_features = self.inter_chunk_fusion(inter_fusion_input)
        elif self.inter_chunk_fusion_return_type == "intermediate_features":
            (
                fused_features,
        ...
        for chunk_idx in range(num_chunks):
            ...
            # 用pose head预测chunk2world pose
            chunk_pose_output = self.inter_chunk_pose_head(
                PredictionHeadInput(last_feature=ref_view_feature)
            )
            chunk_pose_output = self.pose_adaptor(
                AdaptorInput(
                    adaptor_feature=chunk_pose_output.decoded_channels,
                    output_shape_hw=img_shape,
                )
            )
    ...
    def infer(
        ...
        with torch.autocast("cuda", enabled=bool(use_amp), dtype=amp_dtype):
            preds = self.forward(
                processed_views, num_chunks=2, memory_efficient_inference=memory_efficient_inference
            )
