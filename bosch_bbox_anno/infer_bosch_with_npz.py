#!/usr/bin/env python3
"""
Modified Bosch COLMAP inference script that saves both NPZ and GLB files.

This script is based on the original infer_bosch_data_chunk_new_data.py
but adds NPZ file saving functionality for downstream processing.
"""
...
def save_results_with_npz(predictions, output_dir, sequence_name,
                         depth_validity_check=True, conf_threshold=0.0, max_points=-1):
    """
    Save inference results as both NPZ and GLB files.
    ...
    # Save NPZ file with raw predictions data
    npz_path = os.path.join(output_dir, f"{sequence_name}_predictions.npz")
    print(f"Saving NPZ file to: {npz_path}")
    ...
    print(f"Saved NPZ with {len(pi3_predictions['points'])} points")
...
def main():
    parser = argparse.ArgumentParser(description="Run MapAnything on Bosch COLMAP data (with NPZ output)")
    parser.add_argument(
        "--data_root",
        type=str,
        required=True,
    ...
    if rig_ranges:
        ...
        for group_name, img_data_list in rig_groups.items():
            ...
            if args.chunk_timestamps:
                ...
                # Save NPZ file
                npz_path = os.path.join(args.output_dir, f"{sequence_name}_{group_name}_predictions.npz")
                print(f"Saving NPZ file to: {npz_path}")
                np.savez(npz_path, **predictions)
                print(f"Saved NPZ with {len(all_points)} points")
    ...
    if args.chunk_timestamps:
        ...
        # Save merged results directly as GLB and NPZ
        predictions = {
            'points': all_points,
            'images': all_images,
            'camera_poses': all_camera_poses,
        ...
        # Save NPZ file
        npz_path = os.path.join(args.output_dir, f"{sequence_name}_predictions.npz")
        print(f"Saving NPZ file to: {npz_path}")
        np.savez(npz_path, **predictions)
        print(f"Saved NPZ with {len(all_points)} points")
        ...
        # Save results with both NPZ and GLB
        save_results_with_npz(
