...
torchrun --nproc_per_node ${NUM_GPUS} --master_port 29598 \
    scripts/train.py \
    machine=dgx02 \
    ...
    hydra.run.dir='${root_experiments_dir}/mapanything/training/prechunk_freeze_pre_20_layers' \
    model.pretrained='/wekafs/ict/junyiouy/map-anything/checkpoints/facebook_map-anything.pth' 

# 这个主要是把chunk之前的部分freeze住（用torch no_grad)，只训练prechunk之后的部分