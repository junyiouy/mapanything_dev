...
torchrun --nproc_per_node ${NUM_GPUS} --master_port 29507 \
    scripts/train.py \
    machine=dgx02 \
    dataset=mine_scannetpp_only dataset.num_workers=12 \
    dataset.num_views=24 \
    loss=overall_pairwise_loss_weigh_pm_higher_chunk_poseNptcNscale_only_inter_chunk \
    ...
    hydra.run.dir='${root_experiments_dir}/mapanything/training/fusion_only_tune_w_ref_view_load_weight_scale_variable_view_scannetpp_new'
