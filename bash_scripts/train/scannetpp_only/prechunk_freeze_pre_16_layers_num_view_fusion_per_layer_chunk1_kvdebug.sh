...
torchrun --nproc_per_node ${NUM_GPUS} --master_port 29592 \
    scripts/train.py \
    machine=dgx02 \
    ...
    train_params=freeze_encoder_and_pre_freeze_info \
    ...
    hydra.run.dir='${root_experiments_dir}/mapanything/training/prechunk_freeze_pre_16_layers_num_view_fusion_per_layer_chunk1_kvdebug' \
    model.pretrained='/wekafs/ict/junyiouy/map-anything/checkpoints/facebook_map-anything.pth' 

# 这个在上一个的基础上把kv的归一化加上,并且直接使用mv feature作为kv