...
torchrun --nproc_per_node ${NUM_GPUS} --master_port 29592 \
    scripts/train.py \
    machine=dgx02 \
    dataset=mine_scannetpp_only_overfit dataset.num_workers=12 \
    dataset.num_views=24 \
    loss=overall_pairwise_loss_weigh_pm_higher \
    model=mapanything \
    ...
    hydra.run.dir='${root_experiments_dir}/mapanything/training/mapa_mine_scannetpp_24v_2d_12ipg_8g_rel_overfit'
